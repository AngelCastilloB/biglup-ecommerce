/**
 * @file biglup-color-picker.directive.ts
 *
...
import { BiglupColorPickerComponent } from '../../components/color-picker/biglup-color-picker.component';
import { BiglupColorPickerService }   from '../../services/color-picker/biglup-color-picker.service';
import { BiglupTextDirective }        from './internals/biglup-color-picker-text.directive';
import { BiglupSliderDirective }      from './internals/biglup-color-picker-slider-directive';
 ...
 */
@Directive({
    selector: '[biglupColorPicker]',
    host: {
        '(input)': 'changeInput($event.target.value)',
        '(click)': 'onClick()'
    }
})
export class BiglupColorPickerDirective implements OnInit, OnChanges
{
    @Input('biglupColorPicker')
    ...
    public openDialog()
    {
        if (!this._created)
        {
            ...
            this.compiler
                ...
                .then(factory =>
                {
                    const compFactory = factory.componentFactories.find(x => x.componentType === BiglupColorPickerComponent);
...
@NgModule({
    ...
    declarations: [BiglupColorPickerComponent, BiglupTextDirective, BiglupSliderDirective]
})
